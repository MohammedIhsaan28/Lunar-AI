...
const Message = ({role, content}) => {
  ...
  return (
    <div className='flex flex-col items-start w-full max-w-7xl text-sm'>
        <div className={`flex flex-c\ w-fu\ mb-8 ${role === 'user' && 'items-end'}`}>
            <div className={`group relative flex max-w-2xl py-3 rounded-xl ${role === 'user' ? 'bg-[#53535b] px-5': 'gap-3'}`}>
                ...
                {
                    role === 'user'? (
                        <span className='text-white text-lg text-left'>{content}</span>
                    ):(
                        <>
                        <Image src={assets.logo_icon1} alt='' className= 'h-9 w-9 p-1 border border-white/15 rounded-full' />
                        <div className='space-y-4 w-full overflow-scroll text-lg text-left'>
